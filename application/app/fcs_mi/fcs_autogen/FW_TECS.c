 ...
 * Git Hash                       : 109356e0
 * C/C++ source code generated on : Mon Sep 15 11:50:31 2025
 *
             ...
             *rtu_sTECS_Data_Processing, busControllerTECS *rty_controllerTECS)
{
   ...
   *  Bias: '<Root>/Bias'
   */
  rtb_CastToDouble_hqijlyrn5q = 1000.0 * ((*rtu_engineCmd) + 1.0);

  /* Saturate: '<Root>/Saturation' */
  if (rtb_CastToDouble_hqijlyrn5q > 2000.0) {
    rtb_CastToDouble_hqijlyrn5q = 2000.0;
  } else if (rtb_CastToDouble_hqijlyrn5q < 1000.0) {
    rtb_CastToDouble_hqijlyrn5q = 1000.0;
  } else {
    /* no actions */
  }
   ...
   *  Saturate: '<Root>/Saturation'
   */
  rtb_Divide1 = (rtb_CastToDouble_hqijlyrn5q * 0.13) / 642.06450000000007;

  /* DiscreteIntegrator: '<S2>/Discrete-Time Integrator' */
  if (FW_TECS_DW.DiscreteTimeIntegrator_IC_LOADING != 0U) {
    FW_TECS_DW.DiscreteTimeIntegrator_DSTATE = rtb_Divide1;
  }
   ...
   *  Sum: '<S2>/Sum2'
   */
  rtb_pwm_cmd = (642.06450000000007 * (FW_TECS_DW.DiscreteTimeIntegrator_DSTATE
    - (0.1 * (rtu_sTECS_Data_Processing->gammaRad +
              rtu_sTECS_Data_Processing->VdotDivG)))) / FW_TECS_ConstB.Switch3;
   ...
   *  Switch: '<S6>/Switch'
   */
  if (rtb_pwm_cmd > 2000.0) {
    rtb_Switch2_k5ccdp3up5 = 2000.0;
  } else if (rtb_pwm_cmd < 1050.0) {
    /* Switch: '<S6>/Switch' incorporates:
     *  Constant: '<S2>/Constant1'
     */
    rtb_Switch2_k5ccdp3up5 = 1050.0;
  } else {
    rtb_Switch2_k5ccdp3up5 = rtb_pwm_cmd;
  }
   ...
   *  Gain: '<Root>/Gain1'
   */
  rty_controllerTECS->engineCmd = (0.001 * rtb_Switch2_k5ccdp3up5) - 1.0;

  /* DiscreteIntegrator: '<S2>/Discrete-Time Integrator1' */
  if (FW_TECS_DW.DiscreteTimeIntegrator1_IC_LOADING != 0U) {
   ...
   *  Sum: '<S2>/Add3'
   */
  rtb_Divide1 = FW_TECS_DW.DiscreteTimeIntegrator1_DSTATE - (0.1 *
    (rtu_sTECS_Data_Processing->gammaRad - rtu_sTECS_Data_Processing->VdotDivG));

  /* Saturate: '<S2>/Saturation1' */
  if (rtb_Divide1 > 0.26179938779914941) {
    rtb_theta_cmd = 0.26179938779914941;
  } else if (rtb_Divide1 < -0.26179938779914941) {
    rtb_theta_cmd = -0.26179938779914941;
  } else {
    rtb_theta_cmd = rtb_Divide1;
  }
   ...
   *  Sum: '<S2>/Sum4'
   */
  rtb_pwm_cmd = (real_T)((((rtb_pwm_cmd - rtb_Switch2_k5ccdp3up5) * rtb_Add) <=
    0.0) ? ((int32_T)1) : ((int32_T)0));
  ...
  FW_TECS_DW.DiscreteTimeIntegrator_DSTATE += 0.01 * (rtb_pwm_cmd * (0.5 *
    rtb_Add));
  ...
  FW_TECS_DW.DiscreteTimeIntegrator1_DSTATE += 0.01 * (((real_T)((((rtb_Divide1
    - rtb_theta_cmd) * rtb_Add1) <= 0.0) ? ((int32_T)1) : ((int32_T)0))) * (0.25
    * rtb_Add1));

...
\brief
  ...
  Abstract           : Kerne FAT Filesystem Module
...
#include "d_ff.h"      /* Declarations of FatFs API */
#include "d_diskio.h"    /* Declarations of device I/O functions */
#include "xil_cache.h"
...
#define SS(fs)  ((Uint32_t)FF_MAX_SS)  /* Fixed sector size */
#else
...
#endif
#define GET_FATTIME()  ((Uint32_t)(FF_NORTC_YEAR - 1980) << 25 | (Uint32_t)FF_NORTC_MON << 21 | (Uint32_t)FF_NORTC_MDAY << 16)
#else
#define GET_FATTIME()  d_FAT_Gettime()
#endif
...
#if d_FAT_FS_LOCK != 0
#if FF_FS_READONLY
#error d_FAT_FS_LOCK must be 0 at read-only configuration
#endif
typedef struct {
  d_FAT_Fs_t *fs;    /* Object ID 1, volume (NULL:blank entry) */
...
#define TBL_CT850u  {0x43,0x55,0x45,0x41,0x41,0x41,0x41,0x43,0x45,0x45,0x45,0x49,0x49,0x49,0x41,0x41, \
...
#endif
static d_FAT_Fs_t* FatFs[FF_VOLUMES];  /* Pointer to the filesystem objects (logical drives) */
...
#if d_FAT_FS_LOCK != 0
static FILESEM Files[d_FAT_FS_LOCK];  /* Open object lock semaphores */
#endif
...
#if FF_FS_EXFAT
#define DEF_NAMBUF    Uint16_t lbuf[FF_MAX_LFN+1]; Uint8_t dbuf[MAXDIRB(FF_MAX_LFN)];  /* LFN working buffer and directory entry block scratchpad buffer */
#define INIT_NAMBUF(fs)  { (fs)->lfnbuf = lbuf; (fs)->dirbuf = dbuf; }
#define FREE_NAMBUF()
#else
#define DEF_NAMBUF    Uint16_t lbuf[FF_MAX_LFN+1];  /* LFN working buffer */
...
#if FF_FS_EXFAT
#define DEF_NAMBUF    Uint16_t *lfn;  /* Pointer to LFN working buffer and directory entry block scratchpad buffer */
#define INIT_NAMBUF(fs)  { lfn = ff_memalloc((FF_MAX_LFN+1)*2 + MAXDIRB(FF_MAX_LFN)); if (!lfn) LEAVE_FF(fs, d_FAT_NOT_ENOUGH_CORE); (fs)->lfnbuf = lfn; (fs)->dirbuf = (Uint8_t*)(lfn+FF_MAX_LFN+1); }
#define FREE_NAMBUF()  ff_memfree(lfn)
#else
#define DEF_NAMBUF    Uint16_t *lfn;  /* Pointer to LFN working buffer */
#define INIT_NAMBUF(fs)  { lfn = ff_memalloc((FF_MAX_LFN+1)*2); if (!lfn) LEAVE_FF(fs, d_FAT_NOT_ENOUGH_CORE); (fs)->lfnbuf = lfn; }
#define FREE_NAMBUF()  ff_memfree(lfn)
#endif
...
#if d_FAT_CODE_PAGE < 900  /* Static code page configuration (SBCS) */
#define CODEPAGE d_FAT_CODE_PAGE
static const Uint8_t ExCvt[] = TBL_CT850u;
#endif
...
static Uint32_t tchar2uni (  /* Returns character in UTF-16 encoding (>=0x10000 on double encoding unit, 0xFFFFFFFF on decode error) */
...
)
{
  ...
  if (wc != 0) {
    wc = d_FAT_Oem2uni(wc, CODEPAGE);  /* ANSI/OEM ==> Unicode */
    if (wc == 0)
    {
...
static Uint8_t put_utf (  /* Returns number of encoding units written (0:buffer overflow or wrong encoding) */
  Uint32_t chr,  /* UTF-16 encoded character (Double encoding unit char if >=0x10000) */
  TCHAR* buf,  /* Output buffer */
  Uint32_t szb  /* Size of the buffer */
)
{
  Uint16_t wc;

  wc = d_FAT_Uni2oem(chr, CODEPAGE);
...
static int lock_fs (    /* 1:Ok, 0:timeout */
  d_FAT_Fs_t* fs    /* Filesystem object */
)
{
...
static void unlock_fs (
  d_FAT_Fs_t* fs,    /* Filesystem object */
  d_FAT_Result_t res    /* Result code to be returned */
)
{
  if (fs && res != d_FAT_NOT_ENABLED && res != d_FAT_INVALID_DRIVE && res != d_FAT_TIMEOUT) {
...
static d_FAT_Result_t chk_lock (  /* Check if the file can be accessed */
  d_FAT_Dir_t* dp,    /* Directory object pointing the file to be checked */
...
)
{
  ...
  if (i == d_FAT_FS_LOCK) {  /* The object has not been opened */
    return (!be && acc != 2) ? d_FAT_TOO_MANY_OPEN_FILES : d_FAT_OK;  /* Is there a blank entry for new object? */
  }

  /* The object was opened. Reject any open against writing file and all write mode open */
  return (acc != 0 || Files[i].ctr == 0x100) ? d_FAT_LOCKED : d_FAT_OK;
}
...
static int enq_lock (void)  /* Check if an entry is available for a new object */
{
  ...
  for (i = 0; i < d_FAT_FS_LOCK && Files[i].fs; i++)
  {
  }
  return (i == d_FAT_FS_LOCK) ? 0 : 1;
}


static Uint32_t inc_lock (  /* Increment object open counter and returns its index (0:Internal error) */
  d_FAT_Dir_t* dp,  /* Directory object pointing the file to register or increment */
...
)
{
  ...
  if (i == d_FAT_FS_LOCK) {        /* Not opened. Register it as new. */
    for (i = 0; i < d_FAT_FS_LOCK && Files[i].fs; i++)
    {
    }
    if (i == d_FAT_FS_LOCK)
...
static d_FAT_Result_t dec_lock (  /* Decrement object open counter */
  Uint32_t i      /* Semaphore index (1..) */
)
{
  Uint16_t n;
  d_FAT_Result_t res = d_FAT_DISK_ERR;


  if (--i < d_FAT_FS_LOCK) {  /* Index number origin from 0 */
    n = Files[i].ctr;
    if (n == 0x100)
    {
    ...
    res = d_FAT_INT_ERR;      /* Invalid index number */
  }
  return res;
}
...
/*-----------------------------------------------------------------------*/
#if !FF_FS_READONLY
static d_FAT_Result_t sync_window (  /* Returns d_FAT_OK or d_FAT_DISK_ERR */
  d_FAT_Fs_t* fs      /* Filesystem object */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  ...
  if (fs->wflag) {  /* Is the disk access window dirty */
    if (d_FAT_DiskWrite(fs->pdrv, fs->win, fs->winsect, 1) == RES_OK) {  /* Write back the window */
      fs->wflag = 0;  /* Clear window dirty flag */
      if (fs->winsect - fs->fatbase < fs->fsize) {  /* Is it in the 1st FAT? */
        if (fs->n_fats == 2)
        {
          d_FAT_DiskWrite(fs->pdrv, fs->win, fs->winsect + fs->fsize, 1);  /* Reflect it to 2nd FAT if needed */
        }
      }
      res = d_FAT_OK;
    }
...
static d_FAT_Result_t move_window (  /* Returns d_FAT_OK or d_FAT_DISK_ERR */
  d_FAT_Fs_t* fs,      /* Filesystem object */
...
)
{
  ...
  if (sector != fs->winsect) {  /* Window offset changed? */
    ...
    if (res == d_FAT_OK) {      /* Fill sector window with new data */
#endif
      if (d_FAT_DiskRead(fs->pdrv, fs->win, sector, 1) != RES_OK) {
...
static d_FAT_Result_t sync_fs (  /* Returns d_FAT_OK or d_FAT_DISK_ERR */
  d_FAT_Fs_t* fs    /* Filesystem object */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  ...
  if (res == d_FAT_OK) {
    if (fs->fs_type == FS_FAT32 && fs->fsi_flag == 1) {  /* FAT32: Update FSInfo sector if needed */
      ...
      d_FAT_DiskWrite(fs->pdrv, fs->win, fs->winsect, 1);
      fs->fsi_flag = 0;
    }
    /* Make sure that no pending write process in the lower layer */
    if (d_FAT_DiscIoctl(fs->pdrv, CTRL_SYNC, 0) != RES_OK)
    {
      res = d_FAT_DISK_ERR;
    }
  }
...
static Uint32_t clst2sect (  /* !=0:Sector number, 0:Failed (invalid cluster#) */
  d_FAT_Fs_t* fs,    /* Filesystem object */
...
static Uint32_t get_fat (    /* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x7FFFFFFF:Cluster status */
  d_FAT_ObjId_t* obj,  /* Corresponding object */
  Uint32_t clst    /* Cluster number to get the value */
)
{
  Uint32_t wc, bc;
  Uint32_t val;
  d_FAT_Fs_t *fs = obj->fs;
  ...
  if (clst < 2 || clst >= fs->n_fatent) {  /* Check if in valid range */
    ...
    case FS_FAT12 :
      ...
      if (move_window(fs, fs->fatbase + (bc / SS(fs))) != d_FAT_OK)
      {
        break;
      }
    ...
    case FS_FAT16 :
      if (move_window(fs, fs->fatbase + (clst / (SS(fs) / 2))) != d_FAT_OK)
      {
        break;
      }
    ...
    case FS_FAT32 :
      if (move_window(fs, fs->fatbase + (clst / (SS(fs) / 4))) != d_FAT_OK)
      {
        break;
      }
      val = ld_dword(fs->win + clst * 4 % SS(fs)) & 0x0FFFFFFF;  /* Simple Uint32_t array but mask out upper 4 bits */
      break;
#if FF_FS_EXFAT
    case FS_EXFAT :
      if ((obj->objsize != 0 && obj->sclust != 0) || obj->stat == 0) {  /* Object except root dir must have valid data length */
        Uint32_t cofs = clst - obj->sclust;  /* Offset from start cluster */
        Uint32_t clen = (Uint32_t)((obj->objsize - 1) / SS(fs)) / fs->csize;  /* Number of clusters - 1 */
...
static d_FAT_Result_t put_fat (  /* d_FAT_OK(0):succeeded, !=0:error */
  d_FAT_Fs_t* fs,    /* Corresponding filesystem object */
...
)
{
  Uint32_t bc;
  Uint8_t *p;
  d_FAT_Result_t res = d_FAT_INT_ERR;


  if (clst >= 2 && clst < fs->n_fatent) {  /* Check if in valid range */
    switch (fs->fs_type) {
    case FS_FAT12 :
      bc = (Uint32_t)clst; bc += bc / 2;  /* bc: byte offset of the entry */
...
)
{
  ...
  scl = val = clst; ctr = 0;
  for (;;) {
    if (move_window(fs, fs->database + val / 8 / SS(fs)) != d_FAT_OK)
    {
...
static d_FAT_Result_t change_bitmap (
  d_FAT_Fs_t* fs,  /* Filesystem object */
...
)
{
  ...
  for (;;) {
    if (move_window(fs, sect++) != d_FAT_OK)
    {
      return d_FAT_DISK_ERR;
    }
    do {
      do {
        if (bv == (int)((fs->win[i] & bm) != 0))
        {
          return d_FAT_INT_ERR;  /* Is the bit expected value? */
        }
        fs->win[i] ^= bm;  /* Flip the bit */
        fs->wflag = 1;
        if (--ncl == 0)
        {
          return d_FAT_OK;  /* All bits processed? */
        }
      } while (bm <<= 1);    /* Next bit */
      bm = 1;
...
static d_FAT_Result_t fill_first_frag (
  d_FAT_ObjId_t* obj  /* Pointer to the corresponding object */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  Uint32_t cl, n;
...
static d_FAT_Result_t fill_last_frag (
  d_FAT_ObjId_t* obj,  /* Pointer to the corresponding object */
...
static d_FAT_Result_t remove_chain (  /* d_FAT_OK(0):succeeded, !=0:error */
  d_FAT_ObjId_t* obj,    /* Corresponding object */
...
)
{
  ...
  {
    return d_FAT_INT_ERR;  /* Check if in valid range */
  }
  ...
  do {
    ...
    if (nxt == 1)
    {
      return d_FAT_INT_ERR;  /* Internal error? */
    }
    if (nxt == 0xFFFFFFFF)
    {
      return d_FAT_DISK_ERR;  /* Disk error? */
    }
    if (!FF_FS_EXFAT || fs->fs_type != FS_EXFAT) {
    ...
    if (ecl + 1 == nxt) {  /* Is next cluster contiguous? */
      ...
      d_FAT_DiscIoctl(fs->pdrv, CTRL_TRIM, rt);    /* Inform device the data in the block is no longer needed */
#endif
      scl = ecl = nxt;
    }
#endif
...
)
{
  ...
  if (fs->fs_type == FS_EXFAT) {  /* On the exFAT volume */
    ...
    if (res == d_FAT_INT_ERR)
    {
      return 1;
    }
    if (res == d_FAT_DISK_ERR)
    {
      return 0xFFFFFFFF;
    }
  ...
  {  /* On the FAT/FAT32 volume */
    ...
    if (res == d_FAT_OK && clst != 0) {
      res = put_fat(fs, clst, ncl);    /* Link it from the previous one if needed */
    }
  }

  if (res == d_FAT_OK) {      /* Update FSINFO if function succeeded. */
    fs->last_clst = ncl;
    ...
    ncl = (res == d_FAT_DISK_ERR) ? 0xFFFFFFFF : 1;  /* Failed. Generate error status */
  }
...
static Uint32_t clmt_clust (  /* <2:Error, >=2:Cluster number */
  d_FAT_File_t* fp,    /* Pointer to the file object */
  d_FAT_FSIZE_t ofs    /* File offset to be converted to cluster# */
)
{
  Uint32_t cl, ncl, *tbl;
  d_FAT_Fs_t *fs = fp->obj.fs;
...
#if !FF_FS_READONLY
static d_FAT_Result_t dir_clear (  /* Returns d_FAT_OK or d_FAT_DISK_ERR */
  d_FAT_Fs_t *fs,    /* Filesystem object */
...
)
{
  ...
  if (sync_window(fs) != d_FAT_OK)
  {
    return d_FAT_DISK_ERR;  /* Flush disk access window */
  }
  ...
  for (szb = ((Uint32_t)fs->csize * SS(fs) >= MAX_MALLOC) ? MAX_MALLOC : fs->csize * SS(fs), ibuf = 0; szb > SS(fs) && (ibuf = ff_memalloc(szb)) == 0; szb /= 2)
  {
  }
  if (szb > SS(fs)) {    /* Buffer allocated? */
    mem_set(ibuf, 0, szb);
    szb /= SS(fs);    /* Bytes -> Sectors */
    for (n = 0; n < fs->csize && d_FAT_DiskWrite(fs->pdrv, ibuf, sect + n, szb) == RES_OK; n += szb)
    {
  ...
  return (n == fs->csize) ? d_FAT_OK : d_FAT_DISK_ERR;
}
#endif  /* !FF_FS_READONLY */
...
static d_FAT_Result_t dir_sdi (  /* d_FAT_OK(0):succeeded, !=0:error */
  d_FAT_Dir_t* dp,    /* Pointer to directory object */
  Uint32_t ofs    /* Offset of directory table */
)
{
  Uint32_t csz, clst;
  d_FAT_Fs_t *fs = dp->obj.fs;


  if (ofs >= (Uint32_t)((FF_FS_EXFAT && fs->fs_type == FS_EXFAT) ? MAX_DIR_EX : MAX_DIR) || ofs % SZDIRE) {  /* Check range of offset and alignment */
    return d_FAT_INT_ERR;
  }
  ...
  if (clst == 0) {  /* Static table (root-directory on the FAT volume) */
    ...
    {
      return d_FAT_INT_ERR;  /* Is index out of range? */
    }
    dp->sect = fs->dirbase;
    ...
    while (ofs >= csz) {        /* Follow cluster chain */
      ...
      if (clst < 2 || clst >= fs->n_fatent)
      {
        return d_FAT_INT_ERR;  /* Reached to end of table or internal error */
      }
      ofs -= csz;
    }
...
static d_FAT_Result_t dir_next (  /* d_FAT_OK(0):succeeded, d_FAT_NO_FILE:End of table, d_FAT_DENIED:Could not stretch */
  d_FAT_Dir_t* dp,        /* Pointer to the directory object */
...
)
{
  ...
  if (ofs >= (Uint32_t)((FF_FS_EXFAT && fs->fs_type == FS_EXFAT) ? MAX_DIR_EX : MAX_DIR))
  {
    dp->sect = 0;  /* Disable it if the offset reached the max value */
  }
  if (dp->sect == 0)
  {
    return d_FAT_NO_FILE;  /* Report EOT if it has been disabled */
  }
  ...
  if (ofs % SS(fs) == 0) {  /* Sector changed? */
    ...
    if (dp->clust == 0) {  /* Static table */
      if (ofs / SZDIRE >= fs->n_rootdir) {  /* Report EOT if it reached end of static table */
        dp->sect = 0; return d_FAT_NO_FILE;
      }
    }
    else {          /* Dynamic table */
      if ((ofs / SS(fs) & (fs->csize - 1)) == 0) {  /* Cluster changed? */
        ...
        if (clst >= fs->n_fatent) {          /* It reached end of dynamic table */
          ...
          if (clst == 0)
          {
            return d_FAT_DENIED;      /* No free cluster */
          }
          if (clst == 1)
          {
            return d_FAT_INT_ERR;      /* Internal error */
          ...
          if (dir_clear(fs, clst) != d_FAT_OK)
          {
            return d_FAT_DISK_ERR;  /* Clean up the stretched table */
          }
          if (FF_FS_EXFAT)
          {
          ...
          dp->sect = 0; return d_FAT_NO_FILE;      /* Report EOT */
#endif
        }
...
static d_FAT_Result_t dir_alloc (  /* d_FAT_OK(0):succeeded, !=0:error */
  d_FAT_Dir_t* dp,        /* Pointer to the directory object */
  Uint32_t nent        /* Number of contiguous entries to allocate */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  Uint32_t n;
  ...
  if (res == d_FAT_OK) {
    ...
    do {
      ...
      res = dir_next(dp, 1);
    } while (res == d_FAT_OK);  /* Next entry with table stretch enabled */
  }

  if (res == d_FAT_NO_FILE)
  {
    res = d_FAT_DENIED;  /* No directory entry to allocate */
  }
  return res;
}
...
static Uint32_t ld_clust (  /* Returns the top cluster value of the SFN entry */
  d_FAT_Fs_t* fs,      /* Pointer to the fs object */
...
static int cmp_lfn (    /* 1:matched, 0:not matched */
  const Uint16_t* lfnbuf,  /* Pointer to the LFN working buffer to be compared */
...
)
{
  ...
  for (wc = 1, s = 0; s < 13; s++) {    /* Process all characters in the entry */
    ...
    if (wc != 0) {
      if (i >= FF_MAX_LFN || d_FAT_WToUpper(uc) != d_FAT_WToUpper(lfnbuf[i++])) {  /* Compare it */
...
static void put_lfn (
  const Uint16_t* lfn,  /* Pointer to the LFN */
...
)
{
  ...
  while ((chr = *name++) != 0) {
    chr = (Uint16_t)d_FAT_WToUpper(chr);    /* File name needs to be up-case converted */
...
static void get_xfileinfo (
  Uint8_t* dirb,      /* Pointer to the directory entry block 85+C0+C1s */
  d_FAT_FileInfo_t* fno    /* Buffer to store the extracted file information */
)
{
  Uint16_t wc, hs;
  Uint32_t di, si, nc;
  ...
  while (nc < dirb[XDIR_NumName]) {
    ...
    wc = put_utf((Uint32_t)hs << 16 | wc, &fno->fname[di], FF_LFN_BUF - di);  /* Store it in API encoding */
...
static d_FAT_Result_t load_xdir (  /* d_FAT_INT_ERR: invalid entry block */
  d_FAT_Dir_t* dp          /* Reading directory object pointing top of the entry block to load */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  Uint32_t i, sz_ent;
  ...
  {
    return d_FAT_INT_ERR;  /* Invalid order */
  }
  ...
  if (res == d_FAT_NO_FILE)
  {
    res = d_FAT_INT_ERR;  /* It cannot be */
  }
  if (res != d_FAT_OK)
  {
    return res;
...
static void init_alloc_info (
  d_FAT_Fs_t* fs,    /* Filesystem object */
  d_FAT_ObjId_t* obj  /* Object allocation information to be initialized */
)
{
...
static d_FAT_Result_t load_obj_xdir (
  d_FAT_Dir_t* dp,      /* Blank directory object to be used to access containing directory */
  const d_FAT_ObjId_t* obj  /* Object with its containing directory information */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
...
static d_FAT_Result_t store_xdir (
  d_FAT_Dir_t* dp        /* Pointer to the directory object */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  ...
  return (res == d_FAT_OK || res == d_FAT_DISK_ERR) ? res : d_FAT_INT_ERR;
}
...
static d_FAT_Result_t dir_read (
  d_FAT_Dir_t* dp,    /* Pointer to the directory object */
  int vol      /* Filtered by 0:file/directory or 1:volume label */
)
{
  d_FAT_Result_t res = d_FAT_NO_FILE;
  d_FAT_Fs_t *fs = dp->obj.fs;
  Uint8_t a, c;
#if FF_USE_LFN
  ...
  while (dp->sect) {
    ...
    if (c == 0) {
      res = d_FAT_NO_FILE; break; /* Reached to end of the directory */
    }
#if FF_FS_EXFAT
    if (fs->fs_type == FS_EXFAT) {  /* On the exFAT volume */
      if (FF_USE_LABEL && vol) {
...
static d_FAT_Result_t dir_find (  /* d_FAT_OK(0):succeeded, !=0:error */
  d_FAT_Dir_t* dp          /* Pointer to the directory object with the file name */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs = dp->obj.fs;
  ...
  if (fs->fs_type == FS_EXFAT) {  /* On the exFAT volume */
    ...
    while ((res = dir_read_file(dp)) == d_FAT_OK) {  /* Read an item */
#if FF_MAX_LFN < 255
      ...
      for (nc = fs->dirbuf[XDIR_NumName], di = SZDIRE * 2, ni = 0; nc; nc--, di += 2, ni++) {  /* Compare the name */
        ...
        if (d_FAT_WToUpper(ld_word(fs->dirbuf + di)) != d_FAT_WToUpper(fs->lfnbuf[ni]))
        {
          break;
        }
      }
  ...
  do {
    ...
    c = dp->dir[DIR_Name];
    if (c == 0) { res = d_FAT_NO_FILE; break; }  /* Reached to end of table */
#if FF_USE_LFN    /* LFN configuration */
      ...
      if (a == AM_LFN) {      /* An LFN entry is found */
        if (!(dp->fn[NSFLAG] & NS_NOLFN)) {
          if (c & LLEF) {    /* Is it start of LFN sequence? */
            sum = dp->dir[LDIR_Chksum];
            c &= (Uint8_t)~LLEF; ord = c;  /* LFN start order */
...
static d_FAT_Result_t dir_register (  /* d_FAT_OK:succeeded, d_FAT_DENIED:no free entry or too many SFN collision, d_FAT_DISK_ERR:disk error */
  d_FAT_Dir_t* dp            /* Target directory with object name to be created */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs = dp->obj.fs;
#if FF_USE_LFN    /* LFN configuration */
  Uint32_t n, nlen, nent;
  Uint8_t sn[12], sum;


  if (dp->fn[NSFLAG] & (NS_DOT | NS_NONAME))
  {
    return d_FAT_INVALID_NAME;  /* Check name validity */
  }
  ...
  if (fs->fs_type == FS_EXFAT) {  /* On the exFAT volume */
    ...
    if (dp->obj.stat & 4) {      /* Has the directory been stretched? */
      ...
      if (dp->obj.sclust != 0) {    /* Is it a sub directory? */
        ...
        dp->obj.objsize += (Uint32_t)fs->csize * SS(fs);      /* Increase the directory size by cluster size */
  ...
  if (sn[NSFLAG] & NS_LOSS) {      /* When LFN is out of 8.3 format, generate a numbered name */
    ...
    if (n == 100)
    {
      return d_FAT_DENIED;    /* Abort if too many collisions */
    }
    if (res != d_FAT_NO_FILE)
    {
  ...
  res = dir_alloc(dp, nent);    /* Allocate entries */
  if (res == d_FAT_OK && --nent) {  /* Set LFN entry if needed */
    ...
    if (res == d_FAT_OK) {
      ...
      do {          /* Store LFN entries in bottom first */
        ...
        put_lfn(fs->lfnbuf, dp->dir, (Uint8_t)nent, sum);
        fs->wflag = 1;
        res = dir_next(dp, 0);  /* Next entry */
      } while (res == d_FAT_OK && --nent);
    }
  }
...
static d_FAT_Result_t dir_remove (  /* d_FAT_OK:Succeeded, d_FAT_DISK_ERR:A disk error */
  d_FAT_Dir_t* dp          /* Directory object pointing the entry to be removed */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs = dp->obj.fs;
  ...
  res = (dp->blk_ofs == 0xFFFFFFFF) ? d_FAT_OK : dir_sdi(dp, dp->blk_ofs);  /* Goto top of the entry block if LFN is exist */
  if (res == d_FAT_OK) {
    do {
      res = move_window(fs, dp->sect);
      if (res != d_FAT_OK)
...
static void get_fileinfo (
  d_FAT_Dir_t* dp,      /* Pointer to the directory object */
  d_FAT_FileInfo_t* fno    /* Pointer to the file information to be filled */
)
{
  Uint32_t si, di;
#if FF_USE_LFN
  ...
  {  /* On the FAT/FAT32 volume */
    if (dp->blk_ofs != 0xFFFFFFFF) {  /* Get LFN if available */
      ...
      while (fs->lfnbuf[si] != 0) {
        ...
        wc = put_utf((Uint32_t)hs << 16 | wc, &fno->fname[di], FF_LFN_BUF - di);  /* Store it in UTF-16 or UTF-8 encoding */
...
static Uint32_t get_achar (  /* Get a character and advances ptr */
  ...
  do {
    ...
    for (;;) {
      if (*pp == '?' || *pp == '*') {  /* Wildcard? */
        ...
        {
          return 1;  /* Test new branch (recurs up to number of wildcard blocks in the pattern) */
        }
        nc = (unsigned char)*np;
...
static d_FAT_Result_t create_name (  /* d_FAT_OK: successful, d_FAT_INVALID_NAME: could not create */
  d_FAT_Dir_t* dp,          /* Pointer to the directory object */
...
)
{
  ...
  for (;;) {
    ...
    if (uc == 0xFFFFFFFF)
    {
      return d_FAT_INVALID_NAME;    /* Invalid code or UTF decode error */
    }
    if (uc >= 0x10000)
    {
    ...
    {
      return d_FAT_INVALID_NAME;  /* Reject illegal characters for LFN */
    }
    if (di >= FF_MAX_LFN)
    {
      return d_FAT_INVALID_NAME;  /* Reject too long name */
    }
  ...
  *path = p;              /* Return pointer to the next segment */
  ...
  if (di == 0)
  {
    return d_FAT_INVALID_NAME;  /* Reject null name */
  }
  ...
  for (;;) {
    ...
    if (wc >= 0x80) {  /* Is this a non-ASCII character? */
      ...
      wc = d_FAT_Uni2oem(wc, CODEPAGE);      /* Unicode ==> ANSI/OEM code */
      if (wc & 0x80)
      {
  ...
  for (;;) {
    ...
    if (c == '.' || i >= ni) {    /* End of body or field overflow? */
      if (ni == 11 || c != '.')
      {
        return d_FAT_INVALID_NAME;  /* Field overflow or invalid dot? */
      }
    ...
    if (chk_chr("\"*+,:;<=>\?[]|\x7F", c))
    {
      return d_FAT_INVALID_NAME;  /* Reject illegal chrs for SFN */
    }
    if (IsLower(c))
    {
      c -= 0x20;  /* To upper */
  ...
  *path = p + si;            /* Return pointer to the next segment */
  if (i == 0)
  {
    return d_FAT_INVALID_NAME;  /* Reject nul string */
  }
...
static d_FAT_Result_t follow_path (  /* d_FAT_OK(0): successful, !=0: error code */
  d_FAT_Dir_t* dp,          /* Directory object to return last directory and found object */
...
)
{
  ...
  if ((Uint32_t)*path < ' ') {        /* Null path name is the origin directory itself */
    ...
    for (;;) {
      ...
      ns = dp->fn[NSFLAG];
      if (res != d_FAT_OK) {        /* Failed to find the object */
        if (res == d_FAT_NO_FILE) {  /* Object is not found */
          if (FF_FS_RPATH && (ns & NS_DOT)) {  /* If dot entry is not exist, stay there */
            ...
            if (!(ns & NS_LAST))
            {
              res = d_FAT_NO_PATH;  /* Adjust error code if not last segment */
            }
          }
        }
        break;
...
)
{
  ...
  {
    tc = *tt++;
  } while ((Uint32_t)tc >= (FF_USE_LFN ? ' ' : '!') && tc != ':');  /* Find a colon in the path */
...
#endif
    if ((Uint32_t)i < FF_VOLUMES) {  /* If a volume ID is found, get the drive number and strip it */
      vol = i;    /* Drive number */
  ...
  if (*tp == '/') {
    ...
    } while ((c || (tc != '/' && (Uint32_t)tc >= (FF_USE_LFN ? ' ' : '!'))) && ++i < FF_VOLUMES);  /* Repeat for each ID until pattern match */
...
static Uint8_t check_fs (  /* 0:FAT, 1:exFAT, 2:Valid BS but not FAT, 3:Not a BS, 4:Disk error */
  d_FAT_Fs_t* fs,      /* Filesystem object */
...
static d_FAT_Result_t find_volume (  /* d_FAT_OK(0): successful, !=0: an error occurred */
  const TCHAR** path,      /* Pointer to pointer to the path name (drive number) */
  d_FAT_Fs_t** rfs,        /* Pointer to pointer to the found filesystem object */
  Uint8_t mode          /* !=0: Check write protection for write access */
)
{
  Uint8_t fmt, *pt;
  int vol;
  ...
  *rfs = 0;
  ...
  if (!fs)
  {
    return d_FAT_NOT_ENABLED;    /* Is the filesystem object available? */
  }
#if FF_FS_REENTRANT
  if (!lock_fs(fs))
  {
    return d_FAT_TIMEOUT;  /* Lock the volume */
  }
#endif
  *rfs = fs;              /* Return pointer to the filesystem object */
  ...
  if (fs->fs_type != 0) {        /* If the volume has been mounted */
    stat = d_FAT_DiskStatus(fs->pdrv);
    if (!(stat & STA_NOINIT)) {    /* and the physical drive is kept initialized */
      if (!FF_FS_READONLY && mode && (stat & STA_PROTECT)) {  /* Check write protection if needed */
        return d_FAT_WRITE_PROTECTED;
      }
      return d_FAT_OK;        /* The filesystem object is valid */
    }
  }
  ...
  stat = d_FAT_DiskInitialize(fs->pdrv);  /* Initialize the physical drive */
  if (stat & STA_NOINIT) {       /* Check if the initialization succeeded */
    return d_FAT_NOT_READY;      /* Failed to initialize due to no medium or hard error */
  }
  if (!FF_FS_READONLY && mode && (stat & STA_PROTECT)) { /* Check disk write protection if needed */
    return d_FAT_WRITE_PROTECTED;
  }
#if FF_MAX_SS != FF_MIN_SS        /* Get sector size (multiple sector size cfg only) */
  if (d_FAT_DiscIoctl(fs->pdrv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK)
  {
    return d_FAT_DISK_ERR;
  }
  ...
  if (fmt == 4)
  {
    return d_FAT_DISK_ERR;    /* An error occurred in the disk I/O layer */
  }
  if (fmt >= 2)
  {
    return d_FAT_NO_FILESYSTEM;  /* No FAT volume is found */
  }
  ...
  if (fmt == 1) {
    ...
    {
      return d_FAT_NO_FILESYSTEM;
    }

    if (ld_word(fs->win + BPB_FSVerEx) != 0x100)
    {
      return d_FAT_NO_FILESYSTEM;  /* Check exFAT version (must be version 1.0) */
    }
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (It cannot be handled in 32-bit LBA) */
    }
    ...
    if (fs->n_fats != 1)
    {
      return d_FAT_NO_FILESYSTEM;  /* (Supports only 1 FAT) */
    }

    fs->csize = 1 << fs->win[BPB_SecPerClusEx];    /* Cluster size */
    if (fs->csize == 0)
    {
       return d_FAT_NO_FILESYSTEM;  /* (Must be 1..32768) */
    }

    nclst = ld_dword(fs->win + BPB_NumClusEx);    /* Number of clusters */
    if (nclst > MAX_EXFAT)
    {
      return d_FAT_NO_FILESYSTEM;  /* (Too many clusters) */
    }
    fs->n_fatent = nclst + 2;
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (Volume size must not be smaller than the size required) */
    }
    fs->dirbase = ld_dword(fs->win + BPB_RootClusEx);

    /* Check if bitmap location is in assumption (at the first cluster) */
    if (move_window(fs, clst2sect(fs, fs->dirbase)) != d_FAT_OK)
    {
      return d_FAT_DISK_ERR;
    }
...
#endif  /* FF_FS_EXFAT */
  {
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (BPB_BytsPerSec must be equal to the physical sector size) */
    }
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (Must be 1 or 2) */
    }
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (Must be power of 2) */
    }

    fs->n_rootdir = ld_word(fs->win + BPB_RootEntCnt);  /* Number of root directory entries */
    if (fs->n_rootdir % (SS(fs) / SZDIRE))
    {
      return d_FAT_NO_FILESYSTEM;  /* (Must be sector aligned) */
    }
    ...
    if (nrsv == 0)
    {
      return d_FAT_NO_FILESYSTEM;      /* (Must not be 0) */
    }
    ...
    if (tsect < sysect)
    {
      return d_FAT_NO_FILESYSTEM;  /* (Invalid volume size) */
    }
    ...
    if (fmt == FS_FAT32) {
      ...
      {
        return d_FAT_NO_FILESYSTEM;  /* (Must be FAT32 revision 0.0) */
      }
      if (fs->n_rootdir != 0)
      {
        return d_FAT_NO_FILESYSTEM;  /* (BPB_RootEntCnt must be 0) */
      }
      ...
      if (fs->n_rootdir == 0)
      {
         return d_FAT_NO_FILESYSTEM;  /* (BPB_RootEntCnt must not be 0) */
      }
    ...
    {
      return d_FAT_NO_FILESYSTEM;  /* (BPB_FATSz must not be less than the size needed) */
    }
    ...
    if (fmt == FS_FAT32        /* Allow to update FSInfo only if BPB_FSInfo32 == 1 */
      ...
      && move_window(fs, bsect + 1) == d_FAT_OK)
    {
      fs->fsi_flag = 0;
...
#endif
#if d_FAT_FS_LOCK != 0      /* Clear file lock semaphores */
  clear_lock(fs);
#endif
  return d_FAT_OK;
}
...
static d_FAT_Result_t validate (  /* Returns d_FAT_OK or d_FAT_INVALID_OBJECT */
  d_FAT_ObjId_t* obj,      /* Pointer to the d_FAT_ObjId_t, the 1st member in the d_FAT_File_t/DIR object, to check validity */
  d_FAT_Fs_t** rfs        /* Pointer to pointer to the owner filesystem object to return */
)
{
  d_FAT_Result_t res = d_FAT_INVALID_OBJECT;


  if (obj && obj->fs && obj->fs->fs_type && obj->id == obj->fs->id) {  /* Test if the object is valid */
#if FF_FS_REENTRANT
    if (lock_fs(obj->fs)) {  /* Obtain the filesystem object */
      if (!(d_FAT_DiskStatus(obj->fs->pdrv) & STA_NOINIT)) { /* Test if the phsical drive is kept initialized */
        res = d_FAT_OK;
      } else {
        unlock_fs(obj->fs, d_FAT_OK);
      }
    } else {
      res = d_FAT_TIMEOUT;
    }
  ...
  *rfs = (res == d_FAT_OK) ? obj->fs : 0;  /* Corresponding filesystem object */
  return res;
}
...
d_FAT_Result_t d_FAT_Mount (
  d_FAT_Fs_t* fs,      /* Pointer to the filesystem object (NULL:unmount)*/
  const TCHAR* path,  /* Logical drive number to be mounted/unmounted */
  Uint8_t opt      /* Mode option 0:Do not mount (delayed mount), 1:Mount immediately */
)
{
  d_FAT_Fs_t *cfs;
  int vol;
  ...
  if (fs) {
    ...
    if (!ff_cre_syncobj((Uint8_t)vol, &fs->sobj))
    {
      return d_FAT_INT_ERR;
    }
#endif
  ...
  if (opt == 0)
  {
    return d_FAT_OK;      /* Do not mount now, it will be mounted later */
  }
...
d_FAT_Result_t d_FAT_Open (
  d_FAT_File_t* fp,      /* Pointer to the blank file object */
...
)
{
  ...
  if (res == d_FAT_OK) {
    ...
    if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
      if (res != d_FAT_OK) {          /* No file, create new */
        if (res == d_FAT_NO_FILE) {    /* There is no file to open, create a new entry */
#if d_FAT_FS_LOCK != 0
          res = enq_lock() ? dir_register(&dj) : d_FAT_TOO_MANY_OPEN_FILES;
#else
          res = dir_register(&dj);
#endif
        }
        ...
        if (dj.obj.attr & (AM_RDO | AM_DIR)) {  /* Cannot overwrite it (R/O or DIR) */
          ...
          {
            res = d_FAT_EXIST;  /* Cannot create as new file */
          }
        }
      }
      if (res == d_FAT_OK && (mode & FA_CREATE_ALWAYS)) {  /* Truncate the file if overwrite mode */
#if FF_FS_EXFAT
        if (fs->fs_type == FS_EXFAT) {
          ...
          res = store_xdir(&dj);
          if (res == d_FAT_OK && fp->obj.sclust != 0) {  /* Remove the cluster chain if exist */
      ...
      if (res == d_FAT_OK) {          /* Is the object existing? */
    ...
    if (res == d_FAT_OK) {
      ...
      if ((mode & FA_SEEKEND) && fp->obj.objsize > 0) {  /* Seek to end of file if FA_OPEN_APPEND is specified */
        ...
        for (ofs = fp->obj.objsize; res == d_FAT_OK && ofs > bcs; ofs -= bcs) {
        ...
        fp->clust = clst;
        if (res == d_FAT_OK && ofs % SS(fs)) {  /* Fill sector buffer if not on the sector boundary */
          if ((sc = clst2sect(fs, clst)) == 0) {
            res = d_FAT_INT_ERR;
          } else {
            fp->sect = sc + (Uint32_t)(ofs / SS(fs));
#if !FF_FS_TINY
            if (d_FAT_DiskRead(fs->pdrv, fp->buf, fp->sect, 1) != RES_OK)
            {
              res = d_FAT_DISK_ERR;
            }
#endif
...
)
{
  ...
  *br = 0;  /* Clear read byte counter */
  ...
  if (res != d_FAT_OK || (res = (d_FAT_Result_t)fp->err) != d_FAT_OK)
  {
    LEAVE_FF(fs, res);  /* Check validity */
  }
  if (!(fp->flag & FA_READ))
  {
    LEAVE_FF(fs, d_FAT_DENIED); /* Check access mode */
  }
  ...
  for ( ;  btr;                /* Repeat until btr bytes read */
    ...
    if (fp->fptr % SS(fs) == 0) {      /* On the sector boundary? */
      csect = (Uint32_t)(fp->fptr / SS(fs) & (fs->csize - 1));  /* Sector offset in the cluster */
      if (csect == 0) {          /* On the cluster boundary? */
        ...
        if (clst < 2)
        {
          ABORT(fs, d_FAT_INT_ERR);
        }
        if (clst == 0xFFFFFFFF)
        {
          ABORT(fs, d_FAT_DISK_ERR);
        }
      ...
      if (cc > 0) {            /* Read maximum contiguous sectors directly */
        ...
        if (d_FAT_DiskRead(fs->pdrv, rbuff, sect, cc) != RES_OK)
        {
          ABORT(fs, d_FAT_DISK_ERR);
        }
      ...
      if (fp->sect != sect) {      /* Load data sector if not in cache */
        ...
        if (fp->flag & FA_DIRTY) {    /* Write-back dirty sector cache */
          if (d_FAT_DiskWrite(fs->pdrv, fp->buf, fp->sect, 1) != RES_OK)
          {
            ABORT(fs, d_FAT_DISK_ERR);
          }
...
#endif
        if (d_FAT_DiskRead(fs->pdrv, fp->buf, sect, 1) != RES_OK)
        {
           ABORT(fs, d_FAT_DISK_ERR);  /* Fill sector cache */
        }
      }
#endif
      fp->sect = sect;
    }
    rcnt = SS(fs) - (Uint32_t)fp->fptr % SS(fs);  /* Number of bytes left in the sector */
    if (rcnt > btr)
    {
      rcnt = btr;          /* Clip it by btr if needed */
    }
#if FF_FS_TINY
    if (move_window(fs, fp->sect) != d_FAT_OK)
    {
      ABORT(fs, d_FAT_DISK_ERR);  /* Move sector window */
    }
...
)
{
  ...
  *bw = 0;  /* Clear write byte counter */
  ...
  if ((!FF_FS_EXFAT || fs->fs_type != FS_EXFAT) && (Uint32_t)(fp->fptr + btw) < (Uint32_t)fp->fptr) {
  ...
  for ( ;  btw;              /* Repeat until all data written */
    ...
    if (fp->fptr % SS(fs) == 0) {    /* On the sector boundary? */
...
#if FF_FS_TINY
      if (fs->winsect == fp->sect && sync_window(fs) != d_FAT_OK)
      {
        ABORT(fs, d_FAT_DISK_ERR);  /* Write-back sector cache */
      }
#else
      ...
      if (cc > 0) {          /* Write maximum contiguous sectors directly */
        ...
        if (d_FAT_DiskWrite(fs->pdrv, wbuff, sect, cc) != RES_OK)
        {
          ABORT(fs, d_FAT_DISK_ERR);
        }
#if FF_FS_MINIMIZE <= 2
      ...
      if (fp->sect != sect &&     /* Fill sector cache with file data */
        ...
        d_FAT_DiskRead(fs->pdrv, fp->buf, sect, 1) != RES_OK) {
          ABORT(fs, d_FAT_DISK_ERR);
      }
#endif
      fp->sect = sect;
    }
    wcnt = SS(fs) - (Uint32_t)fp->fptr % SS(fs);  /* Number of bytes left in the sector */
    if (wcnt > btw)
    {
...
d_FAT_Result_t d_FAT_Close (
  d_FAT_File_t* fp    /* Pointer to the file object to be closed */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs;
...
#if !FF_FS_READONLY
  res = d_FAT_Sync(fp);          /* Flush cached data */
  if (res == d_FAT_OK)
#endif
  {
...
#endif
#if FF_FS_REENTRANT
      unlock_fs(fs, d_FAT_OK);    /* Unlock volume */
#endif
    }
  }
  return res;
...
)
{
  ...
  if (res == d_FAT_OK) {
    ...
    if (res == d_FAT_OK) {          /* Follow completed */
      if (dj.fn[NSFLAG] & NS_NONAME) {  /* Is it the start directory itself? */
        ...
        if (dj.obj.attr & AM_DIR) {  /* It is a sub-directory */
          ...
          res = d_FAT_NO_PATH;    /* Reached but a file */
        }
      }
    }
    FREE_NAMBUF();
...
#if FF_FS_RPATH >= 2
d_FAT_Result_t d_FAT_GetCwd (
  TCHAR* buff,  /* Pointer to the directory path */
  Uint32_t len    /* Size of buff in unit of TCHAR */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Dir_t dj;
  ...
  if (res == d_FAT_OK) {
    ...
    if (!FF_FS_EXFAT || fs->fs_type != FS_EXFAT) {  /* (Cannot do getcwd on exFAT and returns root path) */
      ...
      while ((ccl = dj.obj.sclust) != 0) {  /* Repeat while current directory is a sub-directory */
        ...
        if (res == d_FAT_NO_FILE)
        {
          res = d_FAT_INT_ERR;/* It cannot be 'not found'. */
        }
        if (res != d_FAT_OK)
        {
          break;
        ...
        if (i < n + 1) {  /* Insufficient space to store the path name? */
          res = d_FAT_NOT_ENOUGH_CORE; break;
        }
        while (n)
        {
    ...
    if (res == d_FAT_OK) {
      ...
      if (res == d_FAT_OK) {
        ...
        do *tp++ = buff[i++]; {
        do *tp++ = buff[i++];  ;  /* Copy stacked path string */
        do *tp++ = buff[i++]; }
      }
    }
    FREE_NAMBUF();
...
d_FAT_Result_t d_FAT_LSeek (
  d_FAT_File_t* fp,    /* Pointer to the file object */
  d_FAT_FSIZE_t ofs    /* File pointer from top of file */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs;
  ...
  if (res == d_FAT_OK)
  {
    res = (d_FAT_Result_t)fp->err;
  }
#if FF_FS_EXFAT && !FF_FS_READONLY
  if (res == d_FAT_OK && fs->fs_type == FS_EXFAT) {
    res = fill_last_frag(&fp->obj, fp->clust, 0xFFFFFFFF);  /* Fill last fragment on the FAT if needed */
  }
#endif
  if (res != d_FAT_OK)
  ...
  if (fp->cltbl) {  /* Fast seek */
    if (ofs == d_FAT_CREATE_LINKMAP) {  /* Create CLMT */
      tbl = fp->cltbl;
      ...
      *fp->cltbl = ulen;  /* Number of items used */
      if (ulen <= tlen) {
        *tbl = 0;    /* Terminate table */
        ...
        res = d_FAT_NOT_ENOUGH_CORE;  /* Given table size is smaller than required */
      }
    } else {            /* Fast seek */
...
#endif
          if (d_FAT_DiskRead(fs->pdrv, fp->buf, dsc, 1) != RES_OK)
          {
            ABORT(fs, d_FAT_DISK_ERR);  /* Load current sector */
          }
#endif
          fp->sect = dsc;
        }
  ...
  /* Normal Seek */
  {
    ...
    if (ofs > 0) {
      ...
      if (ifptr > 0 &&
        ...
        fp->fptr = (ifptr - 1) & ~(d_FAT_FSIZE_t)(bcs - 1);  /* start from the current cluster */
        ofs -= fp->fptr;
        clst = fp->clust;
...
#endif
      if (d_FAT_DiskRead(fs->pdrv, fp->buf, nsect, 1) != RES_OK)
      {
        ABORT(fs, d_FAT_DISK_ERR);  /* Fill sector cache */
      }
#endif
...
d_FAT_Result_t d_FAT_Opendir (
  d_FAT_Dir_t* dp,      /* Pointer to directory object to create */
  const TCHAR* path  /* Pointer to the directory path */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
...
d_FAT_Result_t d_FAT_CloseDir (
  d_FAT_Dir_t *dp    /* Pointer to the directory object to be closed */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs;
...
d_FAT_Result_t d_FAT_ReadDir (
  d_FAT_Dir_t* dp,      /* Pointer to the open directory object */
  d_FAT_FileInfo_t* fno    /* Pointer to file information to return */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs;
  ...
  if (res == d_FAT_OK) {
    if (!fno) {
      ...
      if (res == d_FAT_NO_FILE)
      {
        res = d_FAT_OK;  /* Ignore end of directory */
      }
      if (res == d_FAT_OK) {        /* A valid entry is found */
        get_fileinfo(dp, fno);    /* Get the object information */
        res = dir_next(dp, 0);    /* Increment index for next */
        if (res == d_FAT_NO_FILE)
        {
          res = d_FAT_OK;  /* Ignore end of directory now */
        }
      }
      FREE_NAMBUF();
    }
...
d_FAT_Result_t d_FAT_FindNext (
  d_FAT_Dir_t* dp,    /* Pointer to the open directory object */
  d_FAT_FileInfo_t* fno  /* Pointer to the file information structure */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  ...
  for (;;) {
    res = d_FAT_ReadDir(dp, fno);    /* Get a directory item */
    if (res != d_FAT_OK || !fno || !fno->fname[0])
    {
...
d_FAT_Result_t d_FAT_FindFirst (
  d_FAT_Dir_t* dp,        /* Pointer to the blank directory object */
  d_FAT_FileInfo_t* fno,      /* Pointer to the file information structure */
...
)
{
  ...
  res = d_FAT_Opendir(dp, path);    /* Open the target directory */
  if (res == d_FAT_OK) {
    res = d_FAT_FindNext(dp, fno);  /* Find the first item */
  }
  return res;
}
...
d_FAT_Result_t d_FAT_GetFree (
  const TCHAR* path,  /* Logical drive number */
  Uint32_t* nclst,    /* Pointer to a variable to return number of free clusters */
  d_FAT_Fs_t** fatfs    /* Pointer to return pointer to corresponding filesystem object */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Fs_t *fs;
      ...
      *nclst = fs->free_clst;
      ...
      if (fs->fs_type == FS_FAT12) {  /* FAT12: Scan bit field FAT entries */
        ...
        do {
          ...
          if (stat == 0xFFFFFFFF) { res = d_FAT_DISK_ERR; break; }
          if (stat == 1) { res = d_FAT_INT_ERR; break; }
          if (stat == 0)
          {
            nfree++;
          }
...
)
{
  ...
  if (fp->fptr < fp->obj.objsize) {  /* Process when fptr is not on the eof */
    if (fp->fptr == 0) {  /* When set file size to zero, remove entire cluster chain */
      ...
      if (res == d_FAT_OK && ncl < fs->n_fatent) {
    ...
    fp->flag |= FA_MODIFIED;
#if !FF_FS_TINY
    if (res == d_FAT_OK && (fp->flag & FA_DIRTY)) {
      if (d_FAT_DiskWrite(fs->pdrv, fp->buf, fp->sect, 1) != RES_OK) {
        res = d_FAT_DISK_ERR;
      } else {
        fp->flag &= (Uint8_t)~FA_DIRTY;
...
)
{
  ...
  if (res == d_FAT_OK) {
    ...
    INIT_NAMBUF(fs);
    res = follow_path(&dj, path);    /* Follow the file path */
    if (FF_FS_RPATH && res == d_FAT_OK && (dj.fn[NSFLAG] & NS_DOT)) {
      res = d_FAT_INVALID_NAME;      /* Cannot remove dot entry */
    }
#if d_FAT_FS_LOCK != 0
    if (res == d_FAT_OK)
    {
...
#endif
    if (res == d_FAT_OK) {          /* The object is accessible */
      if (dj.fn[NSFLAG] & NS_NONAME) {
        res = d_FAT_INVALID_NAME;    /* Cannot remove the origin directory */
      } else {
        if (dj.obj.attr & AM_RDO) {
          res = d_FAT_DENIED;    /* Cannot remove R/O object */
        }
      }
      if (res == d_FAT_OK) {
#if FF_FS_EXFAT
      ...
      if (res == d_FAT_OK) {
        ...
        if (res == d_FAT_OK && dclst != 0) {  /* Remove the cluster chain if exist */
#if FF_FS_EXFAT
...
)
{
  ...
  if (res == d_FAT_OK) {
    ...
    if (res == d_FAT_OK)
    {
      res = d_FAT_EXIST;    /* Any object with same name is already existing */
    }
    if (FF_FS_RPATH && res == d_FAT_NO_FILE && (dj.fn[NSFLAG] & NS_DOT)) {
      res = d_FAT_INVALID_NAME;
    }
    if (res == d_FAT_NO_FILE) {        /* Can create a new directory */
      sobj.fs = fs;            /* New object id to create a new chain */
      dcl = create_chain(&sobj, 0);    /* Allocate a cluster for the new directory */
      res = d_FAT_OK;
      if (dcl == 0)
      {
        res = d_FAT_DENIED;    /* No space to allocate a new cluster */
      }
      if (dcl == 1)
      {
        res = d_FAT_INT_ERR;
      ...
      tm = GET_FATTIME();
      if (res == d_FAT_OK) {          /* Initialize the new directory table */
        res = dir_clear(fs, dcl);    /* Clean up the new table */
        if (res == d_FAT_OK && (!FF_FS_EXFAT || fs->fs_type != FS_EXFAT)) {  /* Create dot entries (FAT only) */
          dir = fs->win;
      ...
      if (res == d_FAT_OK) {
        ...
        if (fs->fs_type == FS_EXFAT) {  /* Initialize directory entry block */
          ...
          st_dword(fs->dirbuf + XDIR_FileSize, (Uint32_t)fs->csize * SS(fs));  /* File size needs to be valid */
...
)
{
  ...
  d_FAT_Dir_t djo, djn;
  d_FAT_Fs_t *fs;
  Uint8_t buf[FF_FS_EXFAT ? SZDIRE * 2 : SZDIRE], *dir;
  Uint32_t dw;
  DEF_NAMBUF
  ...
  if (res == d_FAT_OK) {
    ...
    if (res == d_FAT_OK && (djo.fn[NSFLAG] & (NS_DOT | NS_NONAME)))
    {
      res = d_FAT_INVALID_NAME;  /* Check validity of name */
    }
#if d_FAT_FS_LOCK != 0
    if (res == d_FAT_OK) {
      res = chk_lock(&djo, 2);
...
#endif
    if (res == d_FAT_OK) {            /* Object to be renamed is found */
#if FF_FS_EXFAT
      if (fs->fs_type == FS_EXFAT) {  /* At exFAT volume */
        ...
        if (res == d_FAT_OK) {            /* Is new name already in use by any other object? */
          res = (djn.obj.sclust == djo.obj.sclust && djn.dptr == djo.dptr) ? d_FAT_NO_FILE : d_FAT_EXIST;
        }
        if (res == d_FAT_NO_FILE) {         /* It is a valid path and no name collision */
...
/* Start of critical section where an interruption can cause a cross-link */
      ...
      {  /* At FAT/FAT32 volume */
        ...
        mem_cpy(&djn, &djo, sizeof (d_FAT_Dir_t));    /* Duplicate the directory object */
        res = follow_path(&djn, path_new);    /* Make sure if new object name is not in use */
        if (res == d_FAT_OK) {            /* Is new name already in use by any other object? */
          res = (djn.obj.sclust == djo.obj.sclust && djn.dptr == djo.dptr) ? d_FAT_NO_FILE : d_FAT_EXIST;
        }
...
)
{
  ...
  if (res == d_FAT_OK) {
    ...
    INIT_NAMBUF(fs);
    res = follow_path(&dj, path);  /* Follow the file path */
    if (res == d_FAT_OK && (dj.fn[NSFLAG] & (NS_DOT | NS_NONAME)))
    {
      res = d_FAT_INVALID_NAME;  /* Check object validity */
    }
    if (res == d_FAT_OK) {
...
d_FAT_Result_t d_FAT_Utime (
  const TCHAR* path,  /* Pointer to the file/directory name */
  const d_FAT_FileInfo_t* fno  /* Pointer to the timestamp to be set */
)
{
  d_FAT_Result_t res = d_FAT_DISK_ERR;
  d_FAT_Dir_t dj;
...
)
{
  ...
  if (res == d_FAT_OK && label) {
       ...
       if (res == d_FAT_OK) {
        ...
        if (fs->fs_type == FS_EXFAT) {
          ...
          for (si = di = hs = 0; si < dj.dir[XDIR_NumLabel]; si++) {  /* Extract volume label from 83 entry */
            ...
            wc = put_utf((Uint32_t)hs << 16 | wc, &label[di], 4);
            if (wc == 0) { di = 0; break; }
            di += wc;
            hs = 0;
          }
    ...
    if (res == d_FAT_NO_FILE) {  /* No label entry and return nul string */
      label[0] = 0;
      res = d_FAT_OK;
    }
  }
...
)
{
  ...
  if (fs->fs_type == FS_EXFAT) {  /* On the exFAT volume */
    mem_set(dirvn, 0, 22);
    di = 0;
    while ((Uint32_t)*label >= ' ') {  /* Create volume label */
      dc = tchar2uni(&label);  /* Get a Unicode character */
      if (dc >= 0x10000) {
        if (dc == 0xFFFFFFFF || di >= 10) {  /* Wrong surrogate or buffer overflow */
          dc = 0;
        } else {
          st_word(dirvn + di * 2, (Uint16_t)(dc >> 16)); di++;
        }
      }
      if (dc == 0 || chk_chr(badchr + 7, (int)dc) || di >= 11) {  /* Check validity of the volume label */
        LEAVE_FF(fs, d_FAT_INVALID_NAME);
      }
      st_word(dirvn + di * 2, (Uint16_t)dc); di++;
    }
  } else
#endif
  {  /* On the FAT/FAT32 volume */
...
#if FF_USE_LFN
      dc = tchar2uni(&label);
      wc = (dc < 0x10000) ? d_FAT_Uni2oem(d_FAT_WToUpper(dc), CODEPAGE) : 0;
#else                  /* ANSI/OEM input */
      wc = (Uint8_t)*label++;
      if (IsLower(wc))
      {
...
#endif
      if (wc == 0 || chk_chr(badchr + 0, (int)wc) || di >= (Uint32_t)((wc >= 0x100) ? 10 : 11)) {  /* Reject invalid characters for volume label */
        LEAVE_FF(fs, d_FAT_INVALID_NAME);
      }
      if (wc >= 0x100)
      {
    ...
    if (dirvn[0] == DDEM)
    {
      LEAVE_FF(fs, d_FAT_INVALID_NAME);  /* Reject illegal name (heading DDEM) */
    }
    while (di && dirvn[di - 1] == ' ')
    {
...
d_FAT_Result_t d_FAT_Expand (
  d_FAT_File_t* fp,    /* Pointer to the file object */
  d_FAT_FSIZE_t fsz,  /* File size to be expanded to */
...
)
{
  ...
  {
    LEAVE_FF(fs, d_FAT_DENIED);
  }
#if FF_FS_EXFAT
  if (fs->fs_type != FS_EXFAT && fsz >= 0x100000000)
  {
    LEAVE_FF(fs, d_FAT_DENIED);  /* Check if in size limit */
  }
#endif
  ...
  if (fs->fs_type == FS_EXFAT) {
    ...
    if (scl == 0)
    {
      res = d_FAT_DENIED;        /* No contiguous cluster block was found */
    }
    if (scl == 0xFFFFFFFF)
    {
      res = d_FAT_DISK_ERR;
    }
    if (res == d_FAT_OK) {  /* A contiguous free area is found */
...
#endif
  {
    ...
    for (;;) {  /* Find a contiguous cluster block */
      ...
      if (n == 1) { res = d_FAT_INT_ERR; break; }
      if (n == 0xFFFFFFFF) { res = d_FAT_DISK_ERR; break; }
      ...
      if (clst == stcl) { res = d_FAT_DENIED; break; }  /* No contiguous cluster? */
    }
    if (res == d_FAT_OK) {  /* A contiguous free area is found */
      if (opt) {    /* Allocate it now */
...
)
{
  ...
  *bf = 0;  /* Clear transfer byte counter */
  ...
  if (btf > remain)
  {
    btf = (Uint32_t)remain;      /* Truncate btf by remaining bytes */
  }
  ...
  for ( ;  btf && (*func)(0, 0);          /* Repeat until all data transferred or stream goes busy */
    ...
    if (move_window(fs, sect) != d_FAT_OK)
    {
      ABORT(fs, d_FAT_DISK_ERR);  /* Move sector window to the file data */
    }
    dbuf = fs->win;
#else
    if (fp->sect != sect) {    /* Fill sector cache with file data */
    ...
    rcnt = (*func)(dbuf + ((Uint32_t)fp->fptr % SS(fs)), rcnt);  /* Forward the file data */
    if (rcnt == 0)
    {
      ABORT(fs, d_FAT_INT_ERR);
    }
...
{
  const Uint32_t n_fats = 1;    /* Number of FATs for FAT/FAT32 volume (1 or 2) */
  const Uint32_t n_rootdir = 512;  /* Number of root directory entries for FAT volume */
  ...
  Uint32_t szb_buf, sz_buf, sz_blk, n_clst, pau, sect, nsect, n;
  Uint32_t b_vol, b_fat, b_data;        /* Base LBA for volume, fat, data */
  Uint32_t sz_vol, sz_rsv, sz_fat, sz_dir;  /* Size for volume, fat, dir, data */
  Uint32_t i;
  int vol;
  DSTATUS stat;
  ...
  stat = d_FAT_DiskInitialize(pdrv);
  if (stat & STA_NOINIT)
  {
    return d_FAT_NOT_READY;
  }
  if (stat & STA_PROTECT)
  {
    return d_FAT_WRITE_PROTECTED;
  }
  if (d_FAT_DiscIoctl(pdrv, GET_BLOCK_SIZE, &sz_blk) != RES_OK || !sz_blk || sz_blk > 32768 || (sz_blk & (sz_blk - 1)))
  {
    sz_blk = 1;  /* Erase block to align data area */
  }
#if FF_MAX_SS != FF_MIN_SS    /* Get sector size of the medium if variable sector size cfg. */
  if (d_FAT_DiscIoctl(pdrv, GET_SECTOR_SIZE, &ss) != RES_OK)
  {
    return d_FAT_DISK_ERR;
  }
  ...
  {
    return d_FAT_INVALID_PARAMETER;  /* Check if au is valid */
  }
  ...
  if (!buf || sz_buf == 0)
  {
    return d_FAT_NOT_ENOUGH_CORE;
  }

  /* Determine where the volume to be located (b_vol, sz_vol) */
  if (FF_MULTI_PARTITION && part != 0) {
    /* Get partition information from partition table in the MBR */
    if (d_FAT_DiskRead(pdrv, buf, 0, 1) != RES_OK)
    {
      LEAVE_MKFS(d_FAT_DISK_ERR);  /* Load MBR */
    }
    if (ld_word(buf + BS_55AA) != 0xAA55)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* Check if MBR is valid */
    }
    pte = buf + (MBR_Table + (part - 1) * SZ_PTE);
    if (pte[PTE_System] == 0)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* No partition? */
    }
    b_vol = ld_dword(pte + PTE_StLba);    /* Get volume start sector */
    sz_vol = ld_dword(pte + PTE_SizLba);  /* Get volume size */
  } else {
    /* Create a single-partition in this function */
    if (d_FAT_DiscIoctl(pdrv, GET_SECTOR_COUNT, &sz_vol) != RES_OK)
    {
      LEAVE_MKFS(d_FAT_DISK_ERR);
    }
    b_vol = (opt & FM_SFD) ? 0 : 63;    /* Volume start sector */
    if (sz_vol < b_vol)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);
    }
    sz_vol -= b_vol;            /* Volume size */
  }
  if (sz_vol < 128)
  {
    LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* Check if volume size is >=128s */
  }
  ...
  do {
    ...
    if (au > 128)
    {
      LEAVE_MKFS(d_FAT_INVALID_PARAMETER);  /* Too large au for FAT/FAT32 */
    }
    ...
    if (!(opt & FM_FAT))
    {
      LEAVE_MKFS(d_FAT_INVALID_PARAMETER);  /* no-FAT? */
    }
    fmt = FS_FAT16;
  } while (0);
  ...
  if (fmt == FS_EXFAT) {  /* Create an exFAT volume */
    Uint32_t szb_bit, szb_case, sum, nb, cl;
    Uint16_t ch, si;
    Uint32_t j, st;
    Uint8_t b;
    ...
    if (sz_vol < 0x1000)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* Too small volume? */
    }
#if FF_USE_TRIM
    tbl[0] = b_vol; tbl[1] = b_vol + sz_vol - 1;  /* Inform the device the volume area may be erased */
    d_FAT_DiscIoctl(pdrv, CTRL_TRIM, tbl);
#endif
    ...
    if (n_clst <16)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);      /* Too few clusters? */
    }
    if (n_clst > MAX_EXFAT)
    {
      LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* Too many clusters? */
    }
    ...
    do {
      switch (st) {
      case 0:
        ch = (Uint16_t)d_FAT_WToUpper(si);  /* Get an up-case char */
        if (ch != si) {
          si++; break;    /* Store the up-case char if exist */
        }
        for (j = 1; (Uint16_t)(si + j) && (Uint16_t)(si + j) == d_FAT_WToUpper((Uint16_t)(si + j)); j++) ;  /* Get run length of no-case block */
        if (j >= 128) {
      ...
      default:
        ch = (Uint16_t)j; si += (Uint16_t)j;  /* Number of chars to skip */
        st = 0;
      }
      ...
      if (si == 0 || i == szb_buf) {    /* Write buffered data when buffer full or end of process */
        n = (i + ss - 1) / ss;
        if (d_FAT_DiskWrite(pdrv, buf, sect, n) != RES_OK)
        {
          LEAVE_MKFS(d_FAT_DISK_ERR);
        }
        sect += n; i = 0;
    ...
    for (n = 0; n < 2; n++) {
      ...
      if (d_FAT_DiskWrite(pdrv, buf, sect++, 1) != RES_OK)
      {
        LEAVE_MKFS(d_FAT_DISK_ERR);
      }
  ...
  {  /* Create an FAT/FAT32 volume */
    do {
      ...
      if (fmt == FS_FAT32) {  /* FAT32 volume */
        ...
        sz_dir = (Uint32_t)n_rootdir * SZDIRE / ss;  /* Rootdir size [sector] */
      }
      ...
      {
        LEAVE_MKFS(d_FAT_MKFS_ABORTED);  /* Too many clusters for FAT12 */
      }
    ...
    if (d_FAT_DiskWrite(pdrv, buf, b_vol, 1) != RES_OK)
    {
      LEAVE_MKFS(d_FAT_DISK_ERR);  /* Write it to the VBR sector */
    }

    /* Create FSINFO record if needed */
    if (fmt == FS_FAT32) {
      (void)d_FAT_DiskWrite(pdrv, buf, b_vol + 6, 1);    /* Write backup VBR (VBR + 6) */
      mem_set(buf, 0, ss);
      ...
      (void)d_FAT_DiskWrite(pdrv, buf, b_vol + 7, 1);    /* Write backup FSINFO (VBR + 7) */
      (void)d_FAT_DiskWrite(pdrv, buf, b_vol + 1, 1);    /* Write original FSINFO (VBR + 1) */
    }

    /* Initialize FAT area */
    mem_set(buf, 0, (Uint32_t)szb_buf);
    ...
    for (i = 0; i < n_fats; i++) {      /* Initialize FATs each */
      ...
      do {  /* Fill FAT sectors */
        n = (nsect > sz_buf) ? sz_buf : nsect;
        if (d_FAT_DiskWrite(pdrv, buf, sect, (Uint32_t)n) != RES_OK)
        {
          LEAVE_MKFS(d_FAT_DISK_ERR);
        }
        mem_set(buf, 0, ss);
  ...
  if (FF_MULTI_PARTITION && part != 0) {  /* Created in the existing partition */
    ...
    if (d_FAT_DiskRead(pdrv, buf, 0, 1) != RES_OK)
    {
      LEAVE_MKFS(d_FAT_DISK_ERR);  /* Read the MBR */
    }
    buf[MBR_Table + (part - 1) * SZ_PTE + PTE_System] = sys;    /* Set system ID */
    if (d_FAT_DiskWrite(pdrv, buf, 0, 1) != RES_OK)
    {
      LEAVE_MKFS(d_FAT_DISK_ERR);  /* Write it back to the MBR */
    }
    ...
    if (!(opt & FM_SFD)) {  /* Create partition table if in FDISK format */
      ...
      pte[PTE_EdSec] = (Uint8_t)(((n >> 2) & 0xC0) | 63);  /* End sector */
      pte[PTE_EdCyl] = (Uint8_t)n;      /* End cylinder */
      st_dword(pte + PTE_StLba, b_vol);  /* Start offset in LBA */
      st_dword(pte + PTE_SizLba, sz_vol);  /* Size in sectors */
      if (d_FAT_DiskWrite(pdrv, buf, 0, 1) != RES_OK)
      {
        LEAVE_MKFS(d_FAT_DISK_ERR);  /* Write it to the MBR */
      }
    }
  }

  if (d_FAT_DiscIoctl(pdrv, CTRL_SYNC, 0) != RES_OK)
  {
    LEAVE_MKFS(d_FAT_DISK_ERR);
  }
...
)
{
  ...
  if (d_FAT_DiscIoctl(pdrv, GET_SECTOR_COUNT, &sz_disk))
  {
    return d_FAT_DISK_ERR;
  }
  ...
  for (i = 0; i < 4; i++, p += SZ_PTE) {
    ...
    if (e_cyl >= tot_cyl)
    {
      LEAVE_MKFS(d_FAT_INVALID_PARAMETER);
    }

    /* Set partition table */
    p[1] = s_hd;            /* Start head */
    p[2] = (Uint8_t)(((b_cyl >> 2) & 0xC0) | 1);  /* Start sector */
    p[3] = (Uint8_t)b_cyl;          /* Start cylinder */
    p[4] = 0x07;            /* System type (temporary setting) */
    p[5] = e_hd;            /* End head */
    p[6] = (Uint8_t)(((e_cyl >> 2) & 0xC0) | 63);  /* End sector */
  ...
  /* Write it to the MBR */
  res = (d_FAT_DiskWrite(pdrv, buf, 0, 1) == RES_OK && d_FAT_DiscIoctl(pdrv, CTRL_SYNC, 0) == RES_OK) ? d_FAT_OK : d_FAT_DISK_ERR;
  LEAVE_MKFS(res);
}
...
)
{
  ...
  while (nc < len) {
    ...
    d_FAT_Read(fp, s, 1, &rc);
    if (rc != 1)
    {
      break;
    }
    wc = s[0];
    dc = d_FAT_Oem2uni(wc, CODEPAGE);
    if (dc == 0)
    {
      continue;
    }
#elif FF_STRF_ENCODE == 1 || FF_STRF_ENCODE == 2   /* Read a character in UTF-16LE/BE */
    d_FAT_Read(fp, s, 2, &rc);
    if (rc != 2)
    {
      break;
    }
    ...
    if (dc >= 0x80) {  /* Multi-byte character? */
      ...
      d_FAT_Read(fp, s, ct, &rc);    /* Get trailing bytes */
      if (rc != ct)
      {
        break;
      }
...
typedef struct {  /* Putchar output buffer and work area */
  d_FAT_File_t *fp;    /* Ptr to the writing file */
...
)
{
  ...
  if (hs != 0) {
    pb->buf[i++] = (Uint8_t)(hs >> 8);
    pb->buf[i++] = (Uint8_t)hs;
    nc++;
  }
  pb->buf[i++] = (Uint8_t)(wc >> 8);
  pb->buf[i++] = (Uint8_t)wc;
  ...
  wc = d_FAT_Uni2oem(wc, CODEPAGE);  /* UTF-16 ==> ANSI/OEM */
  if (wc == 0)
  {
    return;;
  }
  if (wc >= 0x100) {
    pb->buf[i++] = (Uint8_t)(wc >> 8); nc++;
  }
  pb->buf[i++] = (Uint8_t)wc;
#endif
  ...
  if (i >= (int)(sizeof pb->buf) - 4) {  /* Write buffered characters to the file */
    d_FAT_Write(pb->fp, pb->buf, (Uint32_t)i, &n);
    i = (n == (Uint32_t)i) ? 0 : -1;
  }
  pb->idx = i;
  pb->nchr = nc + 1;
...
)
{
  ...
  if (pb->idx >= 0  /* Flush buffered characters to the file */
       && d_FAT_Write(pb->fp, pb->buf, (Uint32_t)pb->idx, &nw) == d_FAT_OK
       && (Uint32_t)pb->idx == nw)
  {
    return pb->nchr;
  }
